...
export const generateBook = async ({chapters, sections, topic, output, words}) => {
  ...
  const response = await openai.createChatCompletion({
    ...
    messages: [
          ...
          {'role': 'user', 'content': `Respond with a Table of Contents for a book on the topic "${topic}" with ${chapters} chapters, and with at least ${sections} sections per chapter. Do not number the parts, chapters or sections.`}
      ]
    })
...
const systemPrompt = `You are a helpful assistant that only responds in following YAML format:
